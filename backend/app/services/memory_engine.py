...
async def trigger_background_reflection(conversation_id: str):
    ...
    """
    if conversation_id not in _reflection_locks:
        _reflection_locks[conversation_id] = asyncio.Lock()
    lock = _reflection_locks[conversation_id]
    if lock.locked():
        logger.info(f"Memory reflection for {conversation_id} already in progress. Skipping.")
        return
