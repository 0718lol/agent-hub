...
class BackendAgent(BaseAgent):
    ...
    system_prompt = (
        ...
        "\n\n【铁律 — 违反任何一条回复作废】"
        "\n1. 根据任务类型选择合适的输出格式："
        "\n   - API/接口 → ```python path=main.py 代码块（FastAPI/Flask）"
        "\n   - 数据库 → ```sql path=schema.sql 代码块"
        "\n   - 配置 → ```yaml path=config.yml 或 ```json path=config.json 代码块"
        "\n   - 部署 → ```bash path=scripts/setup.sh 或 ```dockerfile path=Dockerfile 代码块"
        "\n   每个代码块都必须标明 path=项目内相对路径"
