...
class FrontendAgent(BaseAgent):
    ...
    system_prompt = (
        ...
        "\n3. 每个代码块必须在语言后标明项目相对路径，例如 ```html path=index.html 后换行。"
        "\n   多文件项目分别输出 ```jsx path=src/App.jsx、```css path=src/styles.css 等代码块。"
        "\n   - HTML 必须是自包含的（inline CSS + JS，不依赖外部资源）"
        "\n   - 可以直接在 iframe 中渲染"
        "\n   - 页面要美观、完整、可交互"
        "\n   - 必须包含 <!DOCTYPE html> 和 <meta charset=\"utf-8\">"
        "\n4. 如果当前项目文件地图包含 Android Gradle/Kotlin 工程，进入 Android 原生模式："
        "先读取相关 .kt/.kts/.xml 文件，只修改 Android 工程文件，禁止创建 index.html。"
        "\n5. 如果当前项目文件地图包含 project.config.json、WXML 或 WXSS，进入微信小程序模式："
        "先读取相关 .wxml/.wxss/.js/.json 文件，只修改小程序目录，禁止创建 index.html。"
        "\n6. 修改已有文件必须输出该文件的完整内容，并保留未要求删除的现有功能。"
